<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="queryParams.account" :placeholder="$t('table.user.account')" style="width: 300px;"
        class="filter-item search-item" clearable />
      <label style="color:#909399;font-weight:500;">组织：</label>
      <treeselect v-model="queryParams.orgId" :clear-value-text="$t('common.clear')" :load-options="loadListOptions"
        :multiple="false" :options="orgList" :searchable="true" class="filter-item search-item" placeholder="组织"
        style="width: 250px;" />
      <label style="color:#909399;font-weight:500;">创建时间：</label>
      <el-date-picker v-model="queryParams.timeRange" :range-separator="null"
        class="filter-item search-item date-range-item" end-placeholder="结束日期" format="yyyy-MM-dd HH:mm:ss"
        start-placeholder="开始日期" type="daterange" style="width: 250px;" value-format="yyyy-MM-dd HH:mm:ss" />
      ...
      <el-button style="background-color: #fff;color: #606266;border-radius: 5px;border-color: #DCDFE6;" @click="reset">{{
        $t('table.reset') }}</el-button>
      <el-dropdown v-has-any-permission="['user:add', 'user:delete', 'user:reset', 'user:export']" class="filter-item"
        trigger="click">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item v-has-permission="['user:delete']" @click.native="batchDelete">{{ $t('table.delete')
          }}</el-dropdown-item>
          <el-dropdown-item v-has-permission="['user:reset']" @click.native="resetPassword">{{ $t('table.resetPassword')
          }}</el-dropdown-item>
          <el-dropdown-item v-has-permission="['user:export']" @click.native="exportExcel">{{ $t('table.export')
          }}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <el-card shadow="never" style="margin-top: 10px;">
      <el-table :key="tableKey" ref="table" v-loading="loading" :data="tableData.records"
        :header-cell-style="{ background: '#FCFBFF', border: '0' }" fit highlight-current-row style="width: 100%;"
        ...
        <el-table-column :label="$t('table.operation')" align="center" class-name="small-padding fixed-width"
          ...
          <template slot-scope="{row}">
            ...
            <el-divider direction="vertical"></el-divider>
            <i v-hasPermission="['user:delete']" style="color: #E05635;" @click="singleDelete(row)">删除</i>
            <el-link v-has-no-permission="['user:view', 'user:update', 'user:delete']" class="no-perm"
              style="color: #009EFF;">{{ $t('tips.noPermission') }}</el-link>
      ...
      <pagination v-show="tableData.total > 0" :limit.sync="pagination.size" :page.sync="pagination.current"
    ...
    resetPassword() {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          orderApi.reset({ ids: ids }).then(response => {
            const res = response.data
            if (res.isSuccess) {
              this.$message({
